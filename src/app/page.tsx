...
export default function HomePage() {
  ...
  const [viewingSpecificScrape, setViewingSpecificScrape] = useState<
    string | null
  >(null);
  const [checkLogFilter, setCheckLogFilter] = useState<
    "all" | "changed" | "meaningful"
  >("all");
  const [processingWebsites, setProcessingWebsites] = useState<Set<string>>(
    new Set()
  );
  const [deletingWebsites, setDeletingWebsites] = useState<Set<string>>(
    new Set()
  );
  const [newlyCreatedWebsites, setNewlyCreatedWebsites] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!passwordValidation.isValid) {
      addToast({
        ...
        description:
          passwordValidation.error || "Please check your password requirements",
        duration: 4000,
      });
      setIsAuthenticating(false);
      return;
    ...
    try {
      ...
      const isInvalidAccount =
        errorLower.includes("invalidaccountid") ||
      ...
      if (isInvalidAccount && authMode === "signIn") {
        ...
        if (authMode === "signIn") {
          ...
          addToast({
            ...
            description:
              "Unable to create your account. Please check your password and try again.",
            duration: 4000,
          });
        }
      } else if (
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="text-center lg:text-left">
                ...
                <p className="text-xl text-zinc-600 dark:text-zinc-400">
                  Monitor websites with change tracking
                </p>
              </div>
    ...
    <Layout>
      ...
      <MainContent maxWidth="7xl" className="py-12">
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            {error && <p className="text-sm text-orange-500 mt-2">{error}</p>}
            <p className="text-xs text-gray-500 mt-2">
              Configure monitor type, check intervals, and notifications after
              adding
            </p>
          </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setExpandedPanel(
                            expandedPanel === "websites" ? null : "websites"
                          )
                        }
                        ...
                        title={
                          expandedPanel === "websites" ? "Minimize" : "Expand"
                        }
                      >
                ...
                <div className="divide-y divide-gray-100">
                  {(() => {
                    ...
                    const totalPages = Math.ceil(
                      filteredWebsites.length / ITEMS_PER_PAGE_WEBSITES
                    );
                    const startIndex =
                      (websitesPage - 1) * ITEMS_PER_PAGE_WEBSITES;
                    ...
                    return (
                      <>
                        {paginatedWebsites.map((website) => {
                          ...
                          const isProcessing = processingWebsites.has(
                            website._id
                          );
                          const isDeleting = deletingWebsites.has(website._id);
                          const hasChanged =
                            latestScrape?.changeStatus === "changed";
                            ...
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <div className="flex items-center gap-2">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    setWebsitesPage(websitesPage - 1)
                                  }
              ...
              <div className="bg-white rounded-lg shadow-sm flex flex-col">
                <div className="p-6 border-b flex-shrink-0">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        variant={
                          checkLogFilter === "all" ? "default" : "outline"
                        }
                        size="sm"
                      ...
                      <Button
                        variant={
                          checkLogFilter === "changed" ? "orange" : "outline"
                        }
                        size="sm"
                      ...
                      <Button
                        variant={
                          checkLogFilter === "meaningful" ? "orange" : "outline"
                        }
                        size="sm"
                      ...
                      <Button
                        ...
                        onClick={() =>
                          setExpandedPanel(
                            expandedPanel === "changes" ? null : "changes"
                          )
                        }
                        className="w-8 h-8 p-0 bg-black text-white border-black rounded-[10px] [box-shadow:inset_0px_-2px_0px_0px_#18181b,_0px_1px_6px_0px_rgba(24,_24,_27,_58%)] hover:translate-y-[1px] hover:scale-[0.98] hover:[box-shadow:inset_0px_-1px_0px_0px_#18181b,_0px_1px_3px_0px_rgba(24,_24,_27,_40%)] active:translate-y-[2px] active:scale-[0.97] active:[box-shadow:inset_0px_1px_1px_0px_#18181b,_0px_1px_2px_0px_rgba(24,_24,_27,_30%)] transition-all duration-200"
                        title={
                          expandedPanel === "changes" ? "Minimize" : "Expand"
                        }
                      >
                  ...
                  {selectedWebsiteId && websites && (
                    <div className="flex items-center gap-2 text-sm bg-orange-100 text-orange-800 px-3 py-1 rounded-full inline-flex w-fit">
                      ...
                      <span className="font-medium">
                        {websites.find((w) => w._id === selectedWebsiteId)
                          ?.name || "Unknown"}
                      </span>
                      <button
                        onClick={() => {
                ...
                <div className="min-h-[600px]">
                  {(() => {
                    ...
                    const filteredHistory = allScrapeHistory.filter(
                      (scrape) => {
                        const websiteMatch =
                          !selectedWebsiteId ||
                          scrape.websiteId === selectedWebsiteId;
                        const filterMatch =
                          checkLogFilter === "all" ||
                          (checkLogFilter === "changed" &&
                            scrape.changeStatus === "changed") ||
                          (checkLogFilter === "meaningful" &&
                            scrape.aiAnalysis?.isMeaningfulChange === true);
                        ...
                        const searchMatch =
                          ...
                          scrape.websiteName
                            ?.toLowerCase()
                            .includes(changesSearchQuery.toLowerCase()) ||
                          scrape.title
                            ?.toLowerCase()
                            .includes(changesSearchQuery.toLowerCase()) ||
                          scrape.description
                    ...
                    const totalChangesPages = Math.ceil(
                      filteredHistory.length / ITEMS_PER_PAGE_CHANGES
                    );
                    const changesStartIndex =
                      (changesPage - 1) * ITEMS_PER_PAGE_CHANGES;
                    const changesEndIndex =
                      changesStartIndex + ITEMS_PER_PAGE_CHANGES;
                    const paginatedChanges = filteredHistory.slice(
                      changesStartIndex,
                      changesEndIndex
                    );
                    ...
                    if (filteredHistory.length === 0) {
                      return (
                        <div className="text-center py-8 text-gray-500">
                          ...
                          {selectedWebsiteId ? (
                            <p className="text-sm mt-1">
                              Select a different website or clear the filter
                            </p>
                          ) : (
                            <p className="text-sm mt-1">
                              Click on a website to filter changes
                            </p>
                          )}
                        </div>
                      );
                              ...
                              <div className="flex items-center gap-3">
                                  ...
                                  getFaviconUrl(scrape.websiteUrl) ? (
                                    <img
                                      ...
                                      onError={(e) => {
                                        e.currentTarget.style.display = "none";
                                        e.currentTarget.parentElement!.innerHTML =
                                          '<div class="w-8 h-8 bg-gray-100 rounded flex items-center justify-center"><svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg></div>';
                                      }}
                                    />
                                  ) : (
                                ...
                                <div className="flex items-center gap-3 flex-shrink-0">
                                  {scrape.aiAnalysis && (
                                    <div className="relative group">
                                      ...
                                      <div className="absolute bottom-full right-0 mb-2 p-3 bg-gray-900 text-white text-sm rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10 w-80">
                                        ...
                                        <div
                                          className={`font-medium mb-1 ${scrape.aiAnalysis.isMeaningfulChange ? "text-green-400" : "text-red-400"}`}
                                        >
                                          {
                                            scrape.aiAnalysis
                                  ...
                                  <div className="w-20 flex justify-end">
                                    <span
                                      className={`text-xs px-2 py-1 rounded flex items-center gap-1 w-20 justify-center ${
                                        scrape.changeStatus === "changed"
                                          ? "bg-orange-100 text-orange-800"
                                          : scrape.changeStatus === "checking"
                                            ? "bg-blue-100 text-blue-800"
                                            : scrape.changeStatus === "new"
                  ...
                  <div className="flex-1 overflow-y-auto">
                    {!websites ? (
                      ...
                      <div className="p-8 text-center text-gray-500">
                        ...
                        <p className="text-sm">
                          Add a website URL above to start monitoring
                        </p>
                      </div>
                    ) : (
                      <div className="divide-y">
                        {websites
                          ...
                          .map((website) => {
                            const latestScrape = latestScrapes
                              ? latestScrapes[website._id]
                              : null;
                            const hasChanged =
                              latestScrape?.changeStatus === "changed";
                            const isProcessing = processingWebsites.has(
                            ...
                            const isDeleting = deletingWebsites.has(
                              website._id
                            );
      ...
      {viewingSpecificScrape && allScrapeHistory && (
        ...
        >
          <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
            {(() => {
              const scrape = allScrapeHistory.find(
                (s) => s._id === viewingSpecificScrape
              );
                      ...
                      <div className="p-4">
                        <div className="font-mono text-sm text-gray-100">
                          {diffLines.map((line, index) => {
                            ...
                            if (onlyShowDiff && !isAddition && !isDeletion)
                              return null;
                            if (!onlyShowDiff) {
                                ...
                                className={`px-2 py-0.5 ${
                                  isAddition
                                    ...
                                    : isDeletion
                                      ...
                                      : isContext
                                        ? "bg-gray-800/50 text-gray-300 font-bold"
                                        : isFileHeader
                                          ? "text-gray-400"
                                          : "text-gray-200"
                                }`}
                  ...
                  <div className="p-4 border-t bg-gray-50 flex justify-between items-center">
                    <div className="flex gap-4 text-sm">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span
                          className={`text-green-700 ${onlyShowDiff ? "opacity-50" : ""}`}
                        >
                          Show Added
                        </span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setShowRemovedLines(e.target.checked)
                          }
                          className="h-4 w-4 text-red-600 rounded focus:ring-red-500"
                          disabled={onlyShowDiff}
                        />
                        <span
                          className={`text-red-700 ${onlyShowDiff ? "opacity-50" : ""}`}
                        >
                          Show Removed
                        </span>
                      </label>
                    ...
                    <Button
                      variant="code"
                      size="sm"
                      onClick={() => setViewingSpecificScrape(null)}
                    >
                      Close
                    </Button>
                  </div>
      ...
      {(editingWebsiteId || pendingWebsite) && (
        <WebhookConfigModal
          ...
          onSave={async (config) => {
            if (pendingWebsite) {
              ...
              try {
                ...
                setProcessingWebsites((prev) => new Set([...prev, websiteId]));
                setNewlyCreatedWebsites(
                  (prev) => new Set([...prev, websiteId])
                );
            ...
            editingWebsiteId
              ? {
                  notificationPreference:
                    websites?.find((w) => w._id === editingWebsiteId)
                      ?.notificationPreference || "none",
                  webhookUrl: websites?.find((w) => w._id === editingWebsiteId)
                    ?.webhookUrl,
                  checkInterval:
                    websites?.find((w) => w._id === editingWebsiteId)
                      ?.checkInterval || 60,
          ...
          websiteName={
            pendingWebsite?.name ||
            websites?.find((w) => w._id === editingWebsiteId)?.name ||
            "Website"
          }
        />
      )}
